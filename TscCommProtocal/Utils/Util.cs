...
namespace TscCommProtocal.Utils
{
    public class Util
    {
        ...
        public static string GetTscIpAddress(Node n)
        {
        ...
        /// </summary>
        /// <returns></returns>
        //public static TscData GetTscDataByApplicationCurrentProperties(Node n)
        //{
        ...
        public static string EventDesc2String(uint i, byte type)
        {
            ...
            else if (type == 0x0a)
            {
                ...
                result = result + "修改时间，";
                //增加时间转换
                result = result + Util.ConvertIntDateTime(i).ToString();
            ...
            else if (type == 0x10)
            {
                byte[] bpara = System.BitConverter.GetBytes(i);
                bpara = bpara.Reverse().ToArray();
                result = result + "总线通信异常，系统故障。系统重起，不影响信号机正常运行。";
            }
            return result;
        }
