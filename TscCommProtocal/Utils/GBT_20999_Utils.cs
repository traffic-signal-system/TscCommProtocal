...
namespace TscCommProtocal.Utils
{
    public class GBT_20999_Utils
    {
        ...
        public void SetReportStatus(byte[] byt)
        {
            ...
            b2 = byt[3];
            b3 = byt[3];
            string workModel = Utils.Util.ReportStatusWorkModel(b1);
            string workStatus = Utils.Util.ReportStatusWorkStatus(b2);
            string controlMode = Utils.Util.ReportStatusControlMode(b3);
            ReportTscStatus.sControlModel = controlMode;    //控制模式
            ReportTscStatus.sWorkModel = workModel;         //工作方式  
            ReportTscStatus.sWorkStatus = workStatus;       //工作状态
            ReportTscStatus.iCurrentSchedule = Convert.ToInt32(byt[4]);             //方案
            ReportTscStatus.iCurrentTimePattern = Convert.ToInt32(byt[5]);          //配时
            ReportTscStatus.iCurrentStagePattern = Convert.ToInt32(byt[6]);         //阶段
            ...
            ReportTscStatus.listChannelRedStatus = Utils.Util.ReportStatusLampStatus(channelRedStatus);
            byte[] channelYellowStatus = { byt[15], byt[16], byt[17], byt[18] };//黄灯
            ReportTscStatus.listChannelYellowStatus = Utils.Util.ReportStatusLampStatus(channelYellowStatus);
            byte[] channelGreenStatus = { byt[19], byt[20], byt[21], byt[22] };//绿灯
            ReportTscStatus.listChannelGreenStatus = Utils.Util.ReportStatusLampStatus(channelGreenStatus);
            byte[] channelFlashStatus = { byt[23], byt[24], byt[25], byt[26] };//黄闪
            ReportTscStatus.listChannelFlashStatus = Utils.Util.ReportStatusLampStatus(channelFlashStatus);
            ReportTscStatus.iCycleTime = byt[27];       //周期时长
